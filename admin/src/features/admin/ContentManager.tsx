...
export const ContentManager = () => {
  ...
  const saveMutation = useMutation({
    ...
    onError: (err: any) => {
      ...
      setErrorMessage(
        err.response?.data?.message || 'Failed to save changes. Please verify server connection.'
      );
  ...
  return (
    <div className="space-y-6 text-left font-body w-full max-w-7xl mx-auto pb-16">
      ...
      {success && (
        <div className="p-4 bg-emerald-50 border border-emerald-300 text-emerald-900 rounded-2xl flex items-center gap-3 shadow-xs animate-fade-in">
          <div className="w-8 h-8 rounded-full bg-emerald-500 text-white flex items-center justify-center font-bold">
            <Check size={18} />
          </div>
          <div>
            <h4 className="text-xs font-extrabold uppercase tracking-wider">Changes Published Successfully</h4>
            <p className="text-xs text-emerald-800 font-medium">Your edits have been saved to the database and are now live on the storefront.</p>
          </div>
        </div>
      )}

      {errorMessage && (
        <div className="p-4 bg-red-50 border border-red-300 text-red-900 rounded-2xl flex items-center gap-3 shadow-xs animate-fade-in">
          <div className="w-8 h-8 rounded-full bg-red-500 text-white flex items-center justify-center font-bold">
            !
          </div>
          <div>
            <h4 className="text-xs font-extrabold uppercase tracking-wider">Save Failed</h4>
