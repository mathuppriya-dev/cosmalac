  ...
  return (
    <div class="space-y-6 font-body text-left">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold font-heading text-text-primary">Product Catalog</h1>
          <p class="text-xs text-text-secondary font-body">Add, edit, or remove skin care formulations in the public showcase database.</p>
        </div>
      ...
      {/* Table */}
      {isLoading ? (
        <div class="text-text-secondary text-xs">Loading items...</div>
      ) : (
            ...
            <table class="w-full text-xs text-left border-collapse">
              <thead>
                <tr class="bg-bg-secondary border-b border-border-pink text-text-primary uppercase tracking-wider font-bold">
                  ...
                  <th class="p-4 text-center">Bestseller</th>
                  <th class="p-4 text-center">Actions</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-border-pink/40">
                {products.map((p: any) => (
                  <tr key={p.id || p._id} class="hover:bg-bg-primary/10">
                    <td class="p-4 font-semibold text-text-primary">{p.title}</td>
                    <td class="p-4 text-text-secondary">{p.category}</td>
                    <td class="p-4 text-center">
                      {p.isFeatured ? <Check class="text-rose-gold mx-auto" size={16} /> : <X class="text-muted mx-auto" size={14} />}
                    </td>
                    <td class="p-4 text-center">
                      {p.isBestseller ? <Check class="text-rose-gold mx-auto" size={16} /> : <X class="text-muted mx-auto" size={14} />}
                    </td>
                    <td class="p-4 flex items-center justify-center gap-3">
                      <button onClick={() => handleEditClick(p)} class="p-1.5 text-text-secondary hover:text-rose-gold" title="Edit">
                        <Edit2 size={13} />
                      </button>
                      <button onClick={() => handleDeleteClick(p.id || p._id)} class="p-1.5 text-text-secondary hover:text-red-600" title="Delete">
                        <Trash2 size={13} />
                      </button>
                    </td>
                  </tr>
      ...
      {modalOpen && (
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div class="fixed inset-0 bg-[#2D2D2D]/30 backdrop-blur-sm" onClick={closeModal} />
          ...
          <div class="bg-white border border-border-pink rounded-3xl p-6 md:p-8 max-w-2xl w-full relative z-10 shadow-2xl max-h-[85vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} class="space-y-4 text-xs font-body">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label class="block font-semibold uppercase tracking-wider text-text-secondary mb-1">Category *</label>
                  <select
                    value={form.category}
                    onChange={(e) => setForm({ ...form, category: e.target.value })}
                    class="w-full px-4 py-2 border border-border-pink rounded-xl focus:outline-none focus:border-rose-gold bg-white"
                  >
              ...
              <div>
                <label class="block font-semibold uppercase tracking-wider text-text-secondary mb-1">Short Description *</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label class="block font-semibold uppercase tracking-wider text-text-secondary mb-1">Full Formulation Description *</label>
                <textarea
                  required
                  rows={4}
                  value={form.description}
                  onChange={(e) => setForm({ ...form, description: e.target.value })}
                  class="w-full px-4 py-2 border border-border-pink rounded-xl focus:outline-none focus:border-rose-gold bg-bg-primary/20 resize-none"
                />
              </div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label class="block font-semibold uppercase tracking-wider text-text-secondary mb-1">Ingredients (comma-separated) *</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label class="block font-semibold uppercase tracking-wider text-text-secondary mb-1">Directions *</label>
                  <input
                    type="text"
                    required
              ...
              <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  <label class="block font-semibold uppercase tracking-wider text-text-secondary mb-1">Warnings</label>
                  <input
                    type="text"
                    value={form.warnings}
                ...
                <div>
                  <label class="block font-semibold uppercase tracking-wider text-text-secondary mb-1">Storage</label>
                  <input
                    type="text"
                    value={form.storage}
                ...
                <div>
                  <label class="block font-semibold uppercase tracking-wider text-text-secondary mb-1">Packaging</label>
                  <input
                    type="text"
              ...
              {/* Checkboxes */}
              <div class="flex items-center gap-6 py-2 border-t border-b border-border-pink/40">
              ...
              <div class="flex items-center justify-end gap-3 pt-2">
                ...
                >
                  {saveMutation.isPending ? <Loader2 size={12} class="animate-spin" /> : 'Save Formulation'}
                </button>
              </div>
            </form>
          </div>
