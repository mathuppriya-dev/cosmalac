...
import { Search, SlidersHorizontal, Grid, Sparkles, ShieldCheck, Award, Heart, RefreshCw } from 'lucide-react';
...
export const Products = () => {
  ...
  const dynamicCategories = ['All', ...Array.from(new Set(products.map((p: any) => p.category).filter(Boolean))) as string[]];
        ...
        <section className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-12 space-y-10">
          {isLoading ? (
            ...
            <div className="p-12 text-center bg-white border border-[#D8D2C8] rounded-3xl space-y-4 max-w-md mx-auto shadow-xs">
              ...
              <button
                onClick={() => refetch()}
                className="px-6 py-2.5 bg-[#121110] text-white rounded-full text-xs font-bold uppercase tracking-wider inline-flex items-center gap-2"
              >
