    ...
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-12 font-body">
        ...
        <div className="text-center max-w-xl mx-auto space-y-3">
          <BookOpen className="text-rose-gold mx-auto" size={40} />
          <h1 className="text-4xl font-extrabold text-text-primary font-heading">Cosmalac Science Lab</h1>
        ...
        {/* Filters bar */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white border border-border-pink p-5 rounded-2xl shadow-sm">
          <div className="relative flex-grow max-w-md">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 bg-bg-primary/20 border border-border-pink/70 rounded-xl text-xs focus:outline-none focus:border-rose-gold placeholder:text-muted"
            />
          </div>

          <div className="flex flex-wrap gap-1.5 overflow-x-auto max-w-lg scrollbar-none">
            {allTags.map((tag: any) => (
              <button
                key={tag}
        ...
        {isLoading ? (
          <div className="h-60 bg-white border border-border-pink animate-pulse rounded-3xl" />
          ...
          <div className="space-y-12">
            {/* Featured Post Card */}
            {featuredBlog && (
              <div className="bg-white border border-border-pink rounded-3xl overflow-hidden shadow-sm grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8 group">
                <div className="lg:col-span-7 aspect-video lg:aspect-auto bg-bg-secondary flex items-center justify-center overflow-hidden">
                  <img
                ...
                <div className="lg:col-span-5 p-6 md:p-8 flex flex-col justify-center text-left space-y-4">
                  ...
                  <h2 className="text-2xl sm:text-3xl font-bold font-heading text-text-primary group-hover:text-rose-gold transition-colors duration-300">
                  ...
                  <div className="pt-2">
                    <Link
                      ...
                      className="px-6 py-2.5 bg-text-primary text-bg-primary text-xs font-semibold uppercase tracking-wider rounded-full hover:bg-rose-gold transition-colors inline-block"
                    >
                      Read Full Article
                    </Link>
                  </div>
            ...
            {standardBlogs.length > 0 && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {standardBlogs.map((blog: any) => (
                  <article key={blog.id || blog._id} className="bg-white border border-border-pink rounded-2xl overflow-hidden shadow-sm hover:shadow transition-all duration-300 flex flex-col text-left group">
                    ...
                    <div className="p-5 flex-grow flex flex-col justify-between space-y-3">
                      <div className="space-y-2">
                        <span className="text-[10px] text-rose-gold font-bold uppercase tracking-wider flex items-center gap-1">
                          <Calendar size={10} /> {new Date(blog.publishedAt).toLocaleDateString()}
                        </span>
                        <h3 className="text-base font-bold font-heading text-text-primary group-hover:text-rose-gold transition-colors duration-300 line-clamp-2">
                      ...
                      <div className="pt-3 border-t border-border-pink/40 flex items-center justify-between mt-auto">
                        <span className="text-[10px] text-text-secondary">By {blog.author.split(',')[0]}</span>
                        <Link to={`/blog/${blog.slug}`} className="text-xs font-bold text-rose-gold hover:text-text-primary">
                          Read Info →
                        </Link>
                      </div>
                    </div>
